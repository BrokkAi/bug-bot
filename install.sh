...
fail() {
    printf 'brokk-bug-bot installer: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    releases=https://github.com/BrokkAi/bug-bot/releases
    version=${1:-latest}
    ...
    [ ! -d "$install_dir/bbb" ] || fail 'installation target is a directory'
    temporary=$(mktemp -d)
    staged=
    trap 'rm -rf "$temporary"; if [ -n "$staged" ]; then rm -f "$staged"; fi' 0
    trap 'exit 1' HUP INT TERM

    asset=brokk-bug-bot-$version-$os-$arch.tar.gz
    printf 'Downloading brokk-bug-bot %s for %s/%s...\n' "$version" "$os" "$arch"
    ...
    mkdir -p "$install_dir"
    staged=$(mktemp "$install_dir/.brokk-bug-bot.XXXXXX")
    # Extract only the binary to a fresh file; never follow archive paths or links.
    tar -xzOf "$temporary/$asset" bbb > "$staged" || fail 'could not extract bbb'
    [ -s "$staged" ] || fail 'archive contains an empty binary'
    chmod 755 "$staged"
    mv -f "$staged" "$install_dir/bbb"
    staged=
    printf 'Installed brokk-bug-bot %s to %s/bbb\n' "$version" "$install_dir"
    case ":${PATH:-}:" in
        *":$install_dir:"*) ;;
        *) printf 'Add %s to your PATH, then run bbb --help.\n' "$install_dir" ;;
    esac
}
