...
func (e engine) reviewAndPublish(ctx context.Context, s *State, w checkout, a Agent, c *Candidate) error {
	...
	for refresh := 0; refresh < 5; refresh++ {
		...
		if len(e.config.Verify) > 0 {
			if _, err := osrun.Run(ctx, w.config.Directory, map[string]string{"BUG_COMMIT": s.Scan.Commit, "BUG_FINDING": jsonContext(c.Finding)}, e.config.Verify...); err != nil {
		...
		c.Status = "submitted"
		c.URL = i.URL
		e.log.Info("Created bug issue", "title", c.Finding.Title, "url", c.URL)
		return e.save(s)
	}
