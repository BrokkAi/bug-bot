...
func TestTerminalProcess(t *testing.T) {
	mode := os.Getenv("BBB_TERMINAL_TEST")
	if mode == "" {
		return
	}
	ctx, cancel := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
	defer cancel()
	args := []string{"once", "--config", os.Getenv("BBB_TERMINAL_CONFIG")}
	if mode == "plain" {
	...
	err := executeWithRun(ctx, args, nil, func(ctx context.Context, _ bot.Config, log *slog.Logger, _ bool) error {
		...
		if mode == "error" {
			return errors.New("fixture scan failed")
		}
		return nil
	})
	if mode == "error" {
		if err == nil || !strings.Contains(err.Error(), "fixture scan failed") {
			t.Fatal(err)
		}
...
func TestTerminalLifecycle(t *testing.T) {
	for _, mode := range []string{"q", "ctrl-c", "sigterm", "success", "error", "plain", "json"} {
		t.Run(mode, func(t *testing.T) {
			...
			cmd.Env = append(os.Environ(), "BBB_TERMINAL_TEST="+workerMode, "BBB_TERMINAL_CONFIG="+config, "TERM=tmux-256color", "PATH="+dir+":"+os.Getenv("PATH"))
			...
			if mode == "plain" || mode == "json" {
				...
				if !strings.Contains(text, "\x1b[?1049h") || !strings.Contains(text, "\x1b[?25h\x1b[?2004l\x1b[?1049l") || !strings.Contains(text, "bbb o/r") {
