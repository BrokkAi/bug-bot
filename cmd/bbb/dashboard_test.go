...
func dashboardFixture() *dashboard {
	...
	cfg.GitHub.Repo = "BrokkAi/bug-bot"
	cfg.Branch = "main"
	cfg.Agent.Model = "review-model"
	cfg.StateDirectory = "/tmp/bug-bot-state"
	d := newDashboard(cfg)
	...
	d.update(bot.Progress{Phase: "reviewing", Task: "Verify empty-input panic (batch 2/4)", Commit: strings.Repeat("a", 40), Attempt: 1, MaxAttempts: 3,
		Counts: bot.FindingCounts{Found: 3, Filed: 1, Duplicates: 1, Pending: 1}, Findings: []bot.FindingProgress{
			{ID: "a", Title: "Empty input panics in parser", Status: "submitted", URL: "https://github.com/BrokkAi/bug-bot/issues/42", Body: strings.Repeat("Root cause: parser indexes an empty slice.\n", 20)},
			{ID: "b", Title: "Cancellation leaves a child process alive", Status: "duplicate", URL: "https://github.com/BrokkAi/bug-bot/issues/12"},
			{ID: "c", Title: "Wide text 日本語 👩‍💻 é breaks table sizing", Status: "pending"},
		}})
	d.scans, d.sessions, d.tools = 2, 4, 23
	d.tool = "go test ./..."
	for _, text := range []string{"Reading parser.go", "Reproducing empty-input panic", "Checking existing issue #12"} {
...
func TestDashboardFitsPaneAndUnicode(t *testing.T) {
	...
	for _, want := range []string{"BrokkAi/bug-bot", "main", "REVIEWING", "attempt 1/3", "3 found", "1 filed", "1 dup", "2 scans", "4 agents", "23 tools", "go test ./..."} {
...
func TestDashboardSummaryKeepsLinksAndDryRun(t *testing.T) {
	...
	d.update(bot.Progress{Findings: []bot.FindingProgress{{ID: "dry", Title: "Proposed issue", Status: "dry_run", Body: "Reproduction\nCall parse with no input"}}})
