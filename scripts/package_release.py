...
def archive_name(tag, target):
    return f"brokk-bug-bot-{tag}-{target}.tar.gz"
    ...
    with path.open("wb") as output, gzip.GzipFile(filename="", fileobj=output, mode="wb", mtime=0) as compressed:
        with tarfile.open(fileobj=compressed, mode="w") as tar:
            for name, data in sorted(files.items()):
                ...
                entry.size = len(data)
                entry.mode = 0o755 if name == "bbb" else 0o644
...
def package(tag, directory):
    ...
    with tempfile.TemporaryDirectory() as temp:
        binary = Path(temp) / "bbb"
        for target in TARGETS:
            goos, goarch = target.split("-")
            env = dict(os.environ, CGO_ENABLED="0", GOOS=goos, GOARCH=goarch)
            run("go", "build", "-trimpath", "-buildvcs=false", f"-ldflags=-s -w -X main.version={tag}", "-o", str(binary), "./cmd/bbb", env=env)
            metadata = {"tag": tag, "commit": sha, "target": target}
            name = archive_name(tag, target)
            archive(directory / name, {
                "bbb": binary.read_bytes(),
...
def verify_local(tag, directory, sha):
    ...
    for asset in manifest["assets"]:
        ...
        with tarfile.open(path, "r:gz") as tar:
            ...
            if len(entries) != 4 or {m.name for m in entries} != {"bbb", "LICENSE", "README.md", "BUILD.json"}:
            ...
            if tar.getmember("bbb").mode & 0o111 == 0:
