...
func Discover(ctx context.Context, target, branch string) (Config, error) {
	...
	if info, err := os.Stat(target); err == nil && info.IsDir() {
		...
		if err != nil {
			return cfg, errors.New("not inside a Git repository; run bug-bot in a checkout or pass a repository URL")
		}
		if bare == "true" {
			...
			if remoteName == "" {
				...
				if len(names) == 0 {
					return cfg, errors.New("this repository has no remote; add origin or pass a repository URL to bug-bot")
				}
				if len(names) != 1 {
					return cfg, errors.New("this repository has several remotes and no origin; pass the issue repository URL explicitly")
				}
				remoteName = names[0]
			}
	...
	workspace := filepath.Join(base, "bug-bot", fmt.Sprintf("%s-%x", name, identity[:12]))
